                  ...
                  class WebsiteAnalyticsDashboard {
                    ...
                    fetchRecentLeads() {
                      // In a real implementation, we'd make an API call to get the leads
                      // For now, we'll use mock data with a small delay to simulate network request
                      setTimeout(() => {
                        const recentLeads = this.getMockLeadData();
                        this.updateRecentLeadsTable(recentLeads);
                      }, 800);

                      // In production, you would use fetch to get data from server:
                      /*
                      fetch('/api/recent-leads')
                        .then(response => response.json())
                        .then(data => {
                          this.updateRecentLeadsTable(data);
                        })
                        .catch(error => {
                          console.error('Error fetching recent leads:', error);
                          document.querySelector('#recent-leads-table tbody').innerHTML = 
                            '<tr><td colspan="7" class="error-data">Error loading leads data</td></tr>';
                        });
                      */
                    }

                    updateRecentLeadsTable(leads) {
                      const leadsTableBody = document.querySelector('#recent-leads-table tbody');
                      if (!leadsTableBody) return;

                      if (leads.length === 0) {
                        leadsTableBody.innerHTML = '<tr><td colspan="7" class="info-data">No leads found</td></tr>';
                        return;
                      }
                      ...
                      leadsTableBody.innerHTML = leads.map(lead => `
                        <tr>
                          ...
                          <td>${lead.lead_source || 'Meta'}</td>
                          <td>${lead.appointment_time || 'Not specified'}</td>
                          <td>${lead.notes || 'None'}</td>
                        </tr>
                      `).join('');
                    }
                    ...
                    getMockLeadData() {
                      ...
                      for (let i = 0; i < 10; i++) {
                        ...
                        // Generate random appointment time (1-7 days from now)
                        const apptDate = new Date(now);
                        apptDate.setDate(apptDate.getDate() + Math.floor(Math.random() * 7) + 1);
                        apptDate.setHours(9 + Math.floor(Math.random() * 8)); // Business hours 9am-5pm
                        apptDate.setMinutes(Math.random() < 0.5 ? 0 : 30); // :00 or :30

                        // Format appointment time as YYYY-MM-DD HH:MM
                        const formattedAppt = `${apptDate.getFullYear()}-${String(apptDate.getMonth() + 1).padStart(2, '0')}-${String(apptDate.getDate()).padStart(2, '0')} ${String(apptDate.getHours()).padStart(2, '0')}:${String(apptDate.getMinutes()).padStart(2, '0')}`;
                        ...
                        leads.push({
                          ...
                          lead_source: 'Meta',
                          appointment_time: formattedAppt,
                          notes: 'None',
