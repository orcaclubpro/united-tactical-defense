/**
 * @deprecated This file is kept for reference but should not be used in new code. 
 * Use GoHighLevelAppointmentClient from '../api/GoHighLevelAppointmentClient' instead.
 * 
 * Legacy script to emulate appointment submission to UDT training website
 * Ported from book-appointment.js
 */
...
export async function submitAppointmentRequest(formData: any = null): Promise<AppointmentResult> {
  console.log('submitAppointmentRequest called with:', formData);
  
  // Use provided form data or default data
  ...
  try {
    ...
    console.log('LEGACY: Request would be sent to: https://backend.leadconnectorhq.com/appengine/appointment');
    console.log('LEGACY: Request headers:', options.headers);
    console.log('LEGACY: Request body length:', options.body.length);
    console.log('LEGACY: First 500 chars of body:', options.body.substring(0, 500));
    
    // Return a mock successful response
    return {
      success: true,
      message: 'Appointment request simulation complete',
      data: {
        appointmentId: 'mock-' + Math.random().toString(36).substring(2),
    ...
    return {
      success: false,
      message: 'Error in appointment request simulation',
      error: error.message
    };
  }
}
...
export default { submitAppointmentRequest }; 